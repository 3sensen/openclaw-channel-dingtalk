...
describe("inbound-handler", () => {
  ...
  it("handleDingTalkMessage blocks invalid summary command for non-owner", async () => {
    shared.extractMessageContentMock.mockReturnValueOnce({
      text: "/summary foo",
      messageType: "text",
    });
    ...
    await handleDingTalkMessage({
      ...
      dingtalkConfig: { groupPolicy: "open", allowFrom: ["owner-test-id"] } as any,
      data: {
        msgId: "m_summary_invalid_deny",
        msgtype: "text",
        text: { content: "/summary foo" },
        conversationType: "2",
        conversationId: "cid_group_summary",
        senderId: "user_not_owner",
        senderNick: "访客",
  ...
  it("handleDingTalkMessage returns summary reply for owner", async () => {
    ...
    expect(runtime.channel.reply.finalizeInboundContext).toHaveBeenCalledWith(
      expect.objectContaining({
        SessionKey: expect.stringMatching(/^s1::summary::/),
      }),
    );
  });
