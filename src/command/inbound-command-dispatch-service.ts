...
export async function handleInboundCommandDispatch(
  ...
  const parsedSummaryCommand = parseSummaryCommand(params.extractedText);
  ...
  if (isOwner) {
    if (isSummaryCommandText(params.extractedText) && parsedSummaryCommand.scope === "unknown") {
      await params.sendReply(formatSummaryCommandHelp());
      return true;
    }
    ...
    if (parsedSummaryCommand.scope === "summary") {
      ...
      const mentionNames = resolveSummaryMentionNames(
        parsedSummaryCommand.mentionNames,
        params.senderName,
      );
      const summaryReply = await generateSummaryNarrative({
        ...
        conversationLabel: params.isDirect
          ? `${params.senderName} (${params.senderId})`
          : `${params.groupName} - ${params.senderName}`,
        chatType: params.isDirect ? "direct" : "group",
      ...
      await params.sendReply(summaryReply);
      return true;
    }
