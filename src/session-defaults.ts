...
export async function initSessionDefault(accountId: string, dingtalkConfig: DingTalkConfig, params: {
  storePath: string;
  sessionKey: string;
  log?: LoggerLike
}) {
  ...
  if (firstTurn) {
    ...
    try {
      ...
      params?.log?.warn?.(
        `[DingTalk] failed to initialize reasoning default for ${params.sessionKey}: ${err instanceof Error ? err.message : String(err)
        }`,
      );
    }
  }
...
async function ensureSessionDefault(params: EnsureParams): Promise<boolean> {
  const {
    storePath,
    sessionKey,
    reasoning = "off",
